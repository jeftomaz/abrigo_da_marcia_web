...
- **Testes:** `pnpm verify` roda tudo na ordem do CI (lint → build → pgTAP → E2E); as partes isoladas são `pnpm lint`, `pnpm build`, `pnpm test:db` e `pnpm e2e`. pgTAP e E2E exigem `supabase start`; o E2E sobe os apps e o runtime das Edge Functions sozinho e força o Supabase local, ignorando o `.env` da raiz. Primeira execução: `npx playwright install chromium webkit`.
- **CI:** branches e PRs rodam somente lint/build; após merge na `main`, `deploy-pages.yml` roda pgTAP/E2E, lint/build e só publica com tudo verde. A suíte completa também deve ser executada localmente antes do merge (`pnpm verify`). Suíte vermelha bloqueia entrega — nenhum teste falhando é tolerado como pendência.
...
- Design system: fidelidade aos mockups Photoshop. Tokens (cores, fontes, breakpoints, tipografia) só via `packages/shared/src/theme.css`, importado pelos dois apps — nunca valores hardcoded em componente. Cada `index.css` mantém apenas o que é exclusivo do seu app (cores de estado e animações do admin). Token usado pelos dois vive no compartilhado: quando o `@theme` estava duplicado, os dois divergiram e o mesmo componente de `packages/shared` renderizava diferente em cada app.
...
- Fotos: todo upload usa `compressImage` de `packages/shared` no client e só segue ao Storage com até 500.000 bytes (aceita JPG, PNG ou WebP; preservar a cota free tier). A saída é WebP, ou JPEG onde o navegador não codifica WebP — o Safari devolve PNG silenciosamente quando se pede WebP, e PNG de foto é lossless, o que forçava a imagem a encolher até caber. A verba se gasta primeiro em resolução (teto de 1600 px) e só depois em qualidade (piso de 0.6); `e2e/imagens.spec.ts` trava esse contrato nos três navegadores. Fotos versionadas no repositório (ex.: `apps/public/src/assets/landing_*.jpg`) não passam por esse client — antes de commitar, rodar `scripts/optimize-photo.py` para remover EXIF/GPS, converter para sRGB e recomprimir.
...
- **Eventos:** no máximo 1 ativo + 3 encerrados; rascunhos não contam. Ao publicar o quinto, o evento com `activated_at` mais antigo é exportado por e-mail e excluído antes da ativação; falha no envio preserva tudo. Na remoção manual, o admin encerra, arquiva e só então exclui o evento com exportação por e-mail e auditoria obrigatórias. Usuário reserva produto/número de rifa → recebe código Pix → envia comprovante fora do site (WhatsApp/Instagram) → admin marca como pago. Reserva expira automaticamente (prazo definido pelo admin) e item volta ao catálogo.
