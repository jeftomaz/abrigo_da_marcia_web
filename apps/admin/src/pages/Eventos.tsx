...
export function Eventos() {
  ...
  const handleSetStatus = async (event: FundraisingEvent, status: EventStatus) => {
    ...
    try {
      if (status === 'active') {
        const reachedHistoryLimit = event.status === 'draft'
          && events.filter((storedEvent) => storedEvent.status !== 'draft').length >= 4
        if (reachedHistoryLimit && !eventSettings?.eventExportEmail.trim()) {
          setActionError('Configure o e-mail de exportação antes de publicar o quinto evento.')
          return false
        }
  ...
  return (
    <main className="flex-1 overflow-x-hidden bg-cinza-claro px-4 py-8 text-cinza-escuro sm:px-6 desk:py-4 dark:bg-cinza-escuro dark:text-cinza-claro">
      ...
      {eventConfirmation && (
        <Dialog ariaLabel={eventConfirmation.action === 'publish' ? 'Publicar evento' : eventConfirmation.action === 'end' ? 'Encerrar evento' : eventConfirmation.action === 'archive' ? 'Arquivar evento' : eventConfirmation.event.status === 'draft' ? 'Remover rascunho' : 'Excluir evento'} onClose={() => setEventConfirmation(null)} className="w-full max-w-[46rem] rounded-3xl bg-surface-raised p-8 text-on-surface-raised sm:p-12">
          <h2 className="text-4xl font-medium text-marca">{eventConfirmation.action === 'publish' ? 'Publicar Evento' : eventConfirmation.action === 'end' ? 'Encerrar Evento' : eventConfirmation.action === 'archive' ? 'Arquivar Evento' : eventConfirmation.event.status === 'draft' ? 'Remover Rascunho' : 'Excluir Evento'}</h2>
          ...
          {eventConfirmation.action === 'publish'
