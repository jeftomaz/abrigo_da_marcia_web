...
export function Eventos() {
  ...
  return (
    <main className="flex-1 overflow-x-hidden bg-cinza-claro px-4 py-8 text-cinza-escuro sm:px-6 desk:py-4 dark:bg-cinza-escuro dark:text-cinza-claro">
      ...
      {eventConfirmation && (
        <Dialog ariaLabel={eventConfirmation.action === 'publish' ? 'Publicar evento' : eventConfirmation.action === 'end' ? 'Encerrar evento' : 'Remover evento'} onClose={() => setEventConfirmation(null)} className="w-full max-w-[46rem] rounded-3xl bg-surface-raised p-8 text-on-surface-raised sm:p-12">
          <h2 className="text-4xl font-medium text-marca">{eventConfirmation.action === 'publish' ? 'Publicar Evento' : eventConfirmation.action === 'end' ? 'Encerrar Evento' : eventConfirmation.event.status === 'draft' ? 'Remover Rascunho' : 'Remover Evento'}</h2>
