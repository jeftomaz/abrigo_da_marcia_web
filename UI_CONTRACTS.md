...
|---|---|---|---|
| `Action` | Todo botão, link e CTA | `variant`: `primary`/`secondary`/`neutral` × `-adaptive`/`-inverted`/`-on-brand`, `surface-adaptive` (branco no claro, preto no escuro), mais `ghost` e `ghost-adaptive` (sem fundo). `size`: `default`, `medium`, `small`, `compact`, `card`, `admin-row`, `admin-row-event`, `admin-inline`, `link`, `link-small`. `children` é opcional — sem ele, o nome acessível precisa vir do `aria-label` | **`gap`, `whitespace`, `px`, `py`, `text-*`, `underline`** — vivem em `SIZE_CLASSES`. Ação dentro de card usa `size="card"`: `compact` reserva 80px de padding e o rótulo vaza a pílula. Ação terciária sem fundo usa `ghost` + `link`/`link-small`. Escolha a variante pela superfície **imediata** (contraste), não pelo tema da página |
...
| `SelectField` | Select com rótulo | `variant`: `filter`, `product`, `toolbar`. `toolbar` abre o select nativo diretamente, com rótulo compacto e foco visível; não usa breakpoint de viewport. `label`, `accessibleLabel` | Aparência do controle |
| `DogSortControl` (admin) | Ordenação de Cães e Cuidados > Por cão | Controle “Ordem”; reutiliza `SelectField` com `toolbar`. Recebe `value`/`onChange`; lista única com dez opções | Seleção aplica imediatamente, sem diálogo ou confirmação. Padrão: inclusão recente; empates usam nome pt-BR e ID. Filtros preservados; programas de Cuidados não são reordenados |
...
- **Variantes compartilhadas que aplicam `lg:` só são usadas no público.** No admin, `SelectField` usa `toolbar`, sem `lg:`; o admin vira em `desk` (85rem).
- **“Por cão” permanece sempre expandida.** Exibe apenas disponíveis por padrão; “Mostrar todos os cães”, inicialmente desativado, inclui adotados e falecidos para consulta. Todos exibem os programas na mesma ordem, sem etapa de seleção; ausência de atribuição preserva a posição e aparece como “Não recebe”. Busca e filtro por tag apenas reduzem o conjunto de cães visíveis, sem recolher seus prontuários.
- **Controles das listagens:** Cães mantém Status, Ordem e Novo Cão na mesma linha, com busca em linha própria. Em Cuidados > Por cão, Ordem acompanha a busca na mesma linha, inclusive no mobile.
- **Busca administrativa de cães:** Cães e Cuidados (Agenda/Por cão) reutilizam `getDogSearchText`: nome, tags, sexo, porte, ano de nascimento, idade calculada, status e descrição. Filtros de status e tags continuam limitando os resultados; a Agenda também pesquisa cuidado e programa.
