...
- `todo` **Bloco 3 — `AdminToolbar`.** Unificar busca, filtro e ação de `Caes.tsx` e `Historias.tsx` preservando suas composições. `Eventos` não tem toolbar e fica fora. Reutilizar `SelectField` com `toolbar` (sem `lg:`) e tamanhos existentes de `Action`.
