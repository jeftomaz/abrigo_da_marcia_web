...
- `todo` **Fase 2b — escala tipográfica no admin.** Os 15 tokens `--text-*` fluidos seguem só no público; o admin usa `text-sm` 93×, `text-3xl` 46× e `text-2xl` 29× caindo no padrão do Tailwind, fora da escala da marca. Mover para o tema compartilhado muda a tipografia do admin (menor abaixo de ~640 px, maior acima de ~1400 px) — exige passada visual em 320/393/1024/1920 px, claro e escuro.
